...
const UpdateFoodRecipePage = ({ params }: { params: { id: string } }) => {
  ...
  useEffect(() => {
    db.foodRecipes.get(parseInt(params.id)).then((recipe) => {
      recipe && setFoodRecipe(recipe);
    });
  }, []);
  ...
  const handleDeleteRecipe = async () => {
    try {
      toast
        .promise(db.foodRecipes?.delete(foodRecipe.id), {
          loading: "Removendo receita...",
          success: <b>Receita removida com sucesso!</b>,
          error: <b>Não foi possível remover a receita.</b>,
        })
        .then(() => setTimeout(() => navigation.back(), 1000));
    } catch (error) {
      console.error(error);
  ...
  return (
    <div>
      ...
      <button
        className="w-full py-3 mt-4 bg-[#ca2f2f] rounded-md text-white text-sm hover:bg-[#f03a3a] transition duration-200"
...
export default UpdateFoodRecipePage;
